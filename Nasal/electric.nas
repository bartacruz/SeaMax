...
var Switch = {
    ...
    get_factor: func(volts) {
        ...
        return math.max(0,math.min(1,switch));
    },
...
var Relay = {
    ...
    ##
    # amps: amps draw for the coil
    # 
    new: func(name,amps) {
        var obj = {parents : [Relay]};
        obj.init(name,amps);
        obj.publish();
        return obj;
    },
    init: func(name,amps){
        me.contacts = [];
        me.super(Load,"init",name,amps,nil);
        me.path="/systems/electrical/relays/";
    },
    publish: func{
        me.super(Load,"publish");
        me.set_prop("open", me.voltage > 0);
        me.set_prop("closed", me.voltage == 0);
    },
    normally_open: func() {
        var name=me.name ~ (size(me.contacts)+1) ~ "no";
        var contact = Switch.new(name,me.path~me.name~'/open');
        append(me.contacts,contact);
        return contact;
    },
    normally_closed: func(name) {
        var name=me.name ~ (size(me.contacts)+1) ~ "nc";
        var contact = Switch.new(name,me.path~me.name~'/closed');
        append(me.contacts,contact);
        return contact;
    },
};
